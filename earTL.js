...
let que = [];
let playNow = false;
exec('/mnt/c/Users/ok/Desktop/stn019320/softalk/SofTalk.exe /close');
...
stream.on('tweet',async tweet => {
  ...
  if(tweet.in_reply_to_screen_name != null){
    ...
    console.log(user);
    text = text.replace(/@(.+?)\s/, '@' + user.data[0].name);
  }
  console.log(text);
...
const playTweet = () => {
  ...
  playNow = true;
  exec('/mnt/c/Users/ok/Desktop/stn019320/softalk/SofTalk.exe /X:1 /S:' + speed +' /W:' + text, () => {
    playNow = false;
  });
  if(que.length > 0){
    playTweet();
